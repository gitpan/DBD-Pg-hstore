...
# This allows declaration	use DBD::Pg::hstore ':all';
...
require XSLoader;
XSLoader::load('DBD::Pg::hstore', $VERSION);
...
DBD::Pg::hstore - Perl extension for encoding/decoding postgresql's hstore data type.
...
=head1 SYNOPSIS
  ...
  my $addinfo = DBD::Pg::hstore::decode($row->{addinfo});   #Decoding into perl hashref
  ...
  $dbh->do("update users set addinfo=? where id=?", undef,
     DBD::Pg::hstore::encode($addinfo), 123);
...
None by default.
Can export B<encode> and B<decode> subs:
 use DBD::Pg::hstore qw/encode decode/;
